...
@main_bp.route("/python63we0i0p/cal/<tableName>/<columnName>", methods=['GET'])
def python63we0i0p_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/python63we0i0p/file/download", methods=['GET'])
def python63we0i0p_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/python63we0i0p/file/upload", methods=['POST'])
def python63we0i0p_file_upload():
    '''
    '''
...
@main_bp.route("/python63we0i0p/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def python63we0i0p_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python63we0i0p/follow/<tableName>/<columnName>", methods=['GET'])
def python63we0i0p_follow(tableName, columnName):
    '''
...
@main_bp.route("/python63we0i0p/location", methods=['GET'])
def python63we0i0p_location():
    '''
...
@main_bp.route("/python63we0i0p/matchFace", methods=['GET'])
def python63we0i0p_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/python63we0i0p/option/<tableName>/<columnName>", methods=['GET'])
def python63we0i0p_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python63we0i0p/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def python63we0i0p_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python63we0i0p/sh/<tableName>", methods=['POST'])
def python63we0i0p_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("python63we0i0p_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/python63we0i0p/upload/<fileName>", methods=['GET'])
def python63we0i0p_upload(fileName):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/python63we0i0p/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/python63we0i0p/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
# 爬取数据
@main_bp.route("/python63we0i0p/spider/<tableName>", methods=['GET'])
def schema_spider(tableName):
    msg = {"code": normal_code, "msg": "success", "data": []}

    # Linux
    cmd = "cd /yykj/python/9999/spider9krj0476 && scrapy crawl "+tableName+"Spider -a databaseName=python63we0i0p"
    # Windows
    # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=python63we0i0p"
    os.system(cmd)
